...
-- thêm bất kỳ user nào vào department "Sale" nữa,
-- khi thêm thì hiện ra thông báo
...
BEGIN
    IF OLD.email = "admin@gmail.com" THEN
        SIGNAL SQLSTATE "12345"
        SET MESSAGE_TEXT = "Đây là tài khoản admin, không được xóa";
    END IF;
END $$
DELIMITER ;
...
-- Question 10: Viết trigger chỉ cho phép người dùng chỉ được
-- update, delete các question khi question đó chưa nằm trong exam nào
...
BEGIN
    ...
    IF v_exam_count > 0 THEN
        SIGNAL SQLSTATE "12345"
        SET MESSAGE_TEXT = "Không cho phép delete";
    END IF;
END $$
DELIMITER ;
...
-- nếu phòng ban nào không có user thì sẽ thay đổi giá trị 0
-- thành "Không có User"
WITH c1 AS (
